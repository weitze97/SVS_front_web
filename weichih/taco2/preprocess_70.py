...
output_path = input_path.split('.')[0] + '_pred.txt' #-->"test_pred.txt"

with open(searching_pool_path, 'r') as f: #'r'不曉得啥意
    searching_pool = json.load(f)  #javascript filetype

if os.path.exists(output_path): #如果檔案存在，先把舊檔案移掉?
    os.remove(output_path)

# w/ tonality consideration
dict_tone = {'-':'1', 'ˊ': '2', 'ˇ': '3', 'ˋ': '4', '˙': '5'}  #讓數字聲調可以用符號表達
dict_bopo2pinyin = import_bopo_pinyin(bopo2pinyin_path)

# read input file
list_sent = load_filepaths_and_text(input_path) #把test.txt讀進來
count_rule0, count_rule1, count_rule2, count_rule3 = 0,0,0,0 #宣告count_rule0,1,2,3
for line in list_sent: #第一次宣告line列表
    lyrics, midis, notelens = line[0], line[1], line[2] #歌詞|音高|長度
    ...
    for lyric in lyrics:
        if lyric == 'sil':
            list_char.append('sil') #append代表在list_char末尾加上'sil'
            list_bopo.append('sil')
        else:
            list_char.extend(list(lyric)) #extend代表取出list(lyric)中每個值分別丟進list_char
    ...
    for idx, bopo in enumerate(list_bopo):
        ...
        if 'sil' != bopo:
            assert bopo[-1] in dict_tone.keys()
    ...
    for idx, (midi, notelen) in enumerate(zip(midis.split(), notelens.split())):
        ...
        notelen = proper_round(float(notelen)*frame_per_beat)
